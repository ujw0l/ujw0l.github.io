...
class carouselSite{
 ...
 let bgColor = `rgba(${Math.floor(Math.random() * 256)},${Math.floor(Math.random() * 255)},${Math.floor(Math.random() * 256)},${(Math.floor(Math.random() * 3)+5)/10})`;
       ...
       sidebarOpts.push(docDiv);
        ...
        supportProj.addEventListener('click', ()=>window.open('https://tidelift.com/subscription/pkg/npm-ctc-carousel-js?utm_source=npm-ctc-carousel-js&utm_medium=referral&utm_campaign=readme', '_blank'));
    ...
    loadDemo() {
        ...
        xhttp.onload = function () {
            if (this.status >= 200 && this.status < 400) {
                ...
                var carousel = new imageCarousel('.image_gallery_2,.image_gallery_1',{callBack:el=>el.style.opacity='1'});
